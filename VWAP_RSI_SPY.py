...
class VWAPRSIStrategy:
    def __init__(self, ib, symbol='SPY', exchange='ARCA', currency='USD'):
        """
        Initializes the VWAP RSI Strategy.
        Args:
            ib: An instance of ib_insync.IB.
            symbol (str): The trading symbol (e.g., 'SPY').
            exchange (str): The exchange where the symbol is traded (e.g., 'ARCA' for SPY).
            currency (str): The currency of the symbol (e.g., 'USD' for SPY).
        """
        self.ib = ib
        self.symbol = symbol
        self.size = 300  # Default order size
        ...
        self.contract = Stock(symbol, exchange=exchange, currency=currency)
    ...
    def connect(self):
        ...
        self.ib.connect('127.0.0.1', 7497, clientId=1) #7496 TWS, 4002 Gateway
        ...
        while True:
            try:
                ...
                if next_interval_minute == 60:
                    next_bar = now.replace(minute=0, second=5, microsecond=0) + timedelta(hours=1)
                else:
                    next_bar = now.replace(minute=next_interval_minute, second=5, microsecond=0)
